...
O ebook final está em `public/ebook-equipe-e-consistencia.pdf`, com as dez imagens originais na ordem de `ebook1.png` a `ebook10.png`, uma por página. A LP não oferece mais o botão de download. A equipe comercial entra em contato manualmente pelo WhatsApp informado para encaminhar o ebook. A capa exibida na página é `public/ebook-capa.png`.
...
- No Metrics, o lead guarda a decisão e sua evidência. `crm_contacts` é sincronizada pelo telefone canônico. Marcado concede `whatsapp_marketing`; desmarcado cria contato novo como `unknown` e preserva consentimentos anteriores. O cadastro não dispara mensagens automáticas.
- A chave do Metrics nunca vai ao navegador.
- A tela confirma o registro e informa que o contato será feito pela equipe comercial. Uma falha no cadastro permite **Tentar novamente** com o mesmo ID de submissão.
